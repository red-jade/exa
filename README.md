...
The EXA platform contains a bespoke umbrella build system
for all EXA libraries.

The `exa` repo contains a shared mix utility module (`mix_util.ex`)
to read command arguments and environment variables,
then generate EXA library dependencies. 
The `mix_util.ex` file defines the current set of consistent
...
There are four _scopes_ to build exa libraries:
 ...
 - `tag`  builds from github tagged releases 
    generated from values defined in the exa `mix_util.ex` file
    and then written to the `deps.ex` file
 - `rel` (default) builds from github tagged releases 
    read from the checked-in `deps.ex` file in each EXA library

The development of a new platform release will typically 
go through phases `local` -> `tag` -> `rel`.
The `tag` phase (over)writes the `deps.ex` file.
